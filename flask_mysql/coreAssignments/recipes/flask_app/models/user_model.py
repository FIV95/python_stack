...
class User:
    ...
    def validate(user_data):
        ...
        if len(user_data['first_name']) < 2 or len(user_data['first_name']) > 255:
            print('\n\n\n LINE 33 IN USER_MODEL', '\n\n')
            flash('Min # of Chars: 2, Max # of Chars: 255', 'fname_err')
            is_valid = False

        fnamevalue = user_data.get('first_name')
        if len(fnamevalue) < 1:
            print('\n\n\n LINE 38 IN USER_MODEL', fnamevalue, '\n\n')
            flash('Please Enter a First Name', 'no_fname_err')
            is_valid = False

        lnamevalue = user_data.get('last_name')
        if not lnamevalue:
            flash ('Please Enter a Last Name', 'no_lname_err')
            is_valid = False
    ...
    def select_id_by_email(cls, l_email):
        print('\n\n\--------- LINE 123 USER MODEL L_EMAIL',l_email, '\n\n')
        ...
        if results:
            ...
            flash(f'No user with email {l_email} was not found.','login_err' )
            return None
